...
int queue_push(my_queue *queue, const char *string) {
  if (!(queue->head)) {
    if ((queue->head = (node *)malloc(sizeof(my_queue))) == NULL) {
      perror("[error]");

      return 1;
    }

    if ((queue->head->data = (char *)malloc(sizeof(char) * (strlen(string) + 1))) == NULL) {
      perror("[error]");
      free(queue->head);
      queue->head = NULL;
  ...
  if (queue->head == queue->tail) {
    if ((queue->tail = (node *)malloc(sizeof(my_queue))) == NULL) {
      perror("[error]");

      return 1;
    }

    if ((queue->tail->data = (char *)malloc(sizeof(char) * (strlen(string) + 1))) == NULL) {
      perror("[error]");
      free(queue->tail);
      queue->tail = queue->head;
  ...
  if ((temp = (node *)malloc(sizeof(my_queue))) == NULL) {
    perror("[error]");
